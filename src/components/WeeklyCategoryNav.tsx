import Link from "next/link";
import { CATEGORIES, NewsSourceId } from "@/lib/config";
import { buildWeeklySummaryHref } from "@/lib/url";
import { WeeklyCategoryId } from "@/lib/weekly";

interface WeeklyCategoryNavProps {
  activeId: WeeklyCategoryId;
  activeSourceId?: NewsSourceId | "all";
}

const ALL_CATEGORY = {
  id: "all" as const,
  name: "All News",
  icon: "📰",
};

export function WeeklyCategoryNav({
  activeId,
  activeSourceId = "all",
}: WeeklyCategoryNavProps) {
  const categories = [ALL_CATEGORY, ...CATEGORIES];

  return (
    <nav
      className="scrollbar-hide -mx-4 flex gap-2 overflow-x-auto px-4 pb-1"
      aria-label="Weekly summary categories"
    >
      {categories.map((category) => {
        const isActive = category.id === activeId;
        return (
          <Link
            key={category.id}
            href={buildWeeklySummaryHref(category.id, {
              source: activeSourceId,
            })}
            className={`flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2.5 text-sm font-medium transition-colors ${
              isActive
                ? "bg-slate-900 text-white shadow-sm"
                : "bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50"
            }`}
          >
            <span aria-hidden>{category.icon}</span>
            {category.name}
          </Link>
        );
      })}
    </nav>
  );
}
